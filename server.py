...
# 2. ตรวจสอบสถานะว่า LINE User ID นี้ผูกกับนักเรียนคนไหน และใช้สิทธิ์โหวตไปหรือยัง
@app.get("/api/voter-status")
def get_voter_status(line_id: str = Query(..., description="LINE User ID ที่ต้องการตรวจสอบ")):
    if not line_id:
